...
public:
	CEXP_OVER(COMP_OP comp_op, char num_bytes, void* mess) : m_comp_op(comp_op)
	{
	...
	CEXP_OVER(COMP_OP comp_op) : m_comp_op(comp_op) {}
...
public:
	...
	void do_recv()
	{
		...
		_recv_over.m_wsa_buf.buf = reinterpret_cast<char*>(_recv_over.m_net_buf + _prev_size);
		_recv_over.m_wsa_buf.len = sizeof(_recv_over.m_net_buf) - _prev_size;
	...
	void do_send(int num_bytes, void* mess)
	{
		...
		WSASend(_socket, &ex_over->m_wsa_buf, 1, 0, 0, &ex_over->m_wsa_over, NULL);
	}
};
...
void worker()
{
	for (;;) {
		...
		case OP_ACCEPT: {
			...
			cl._recv_over.m_wsa_buf.buf = reinterpret_cast<char*>(cl._recv_over.m_net_buf);
			cl._recv_over.m_wsa_buf.len = sizeof(cl._recv_over.m_net_buf);
			...
			*(reinterpret_cast<SOCKET*>(exp_over->m_net_buf)) = c_socket;
			AcceptEx(g_s_socket, c_socket, exp_over->m_net_buf + 8, 0, sizeof(SOCKADDR_IN) + 16,
